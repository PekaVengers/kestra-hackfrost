...
const { Goal, SWOTItem, SWOTData } = require("../models/SWOT");
const Kestra = require("../kestra-sdk-main/index");

// Create a new Goal associated with a user

const kestraSDK = new Kestra({
  baseUrl: "http://localhost:8080",
  apiKey: "admin",
});

exports.getGoal = async (goal, time) => {
  if (!goal || !time) {
    throw new Error("Both goal and timeframe are required.");
  }

  try {
    const response = await kestraSDK.workflow.executeWorkflow(
      "company.team",
      "swotGoal",
      { goal, time }
    );
